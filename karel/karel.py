...
from .utils import Tcolors, get_rng, InvalidActionError
...
class Karel(object):
    ...
    def move(self):
        ...
        if not self._front_is_clear():
            raise InvalidActionError('can\'t move. There is a wall in front of Hero')
            success = False
        else:
            self.hero.move()
        return success
    ...
    def pick_marker(self):
        ...
        for i, coord in enumerate(self.markers):
            ...
            raise InvalidActionError('can\'t pick marker from empty location')
            pass
    ...
    def put_marker(self):
        ...
        if not self.hero.holding_markers():
            raise InvalidActionError('can\'t put marker. Hero has none')
            pass
        else:
